...
class RandomRotationFlip(object):
    ...
    def __call__(self, x, is_flow=False):
        ...
        affine_grid = F.affine_grid(M_original_transformed, x.unsqueeze(dim=0).shape, align_corners=True)
        transformed = F.grid_sample(x.unsqueeze(dim=0), affine_grid, align_corners=True)
